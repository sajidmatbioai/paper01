"""
============================================================
 LSTM — 20-Class Classification ONLY
 ...
 Output  : /kaggle/working/lstm_real_multiclass.keras
============================================================
"""
...
MULTI_MODEL = '/kaggle/working/lstm_real_multiclass.keras'
...
# ============================================================
print("=" * 60)
print("  LSTM 20-CLASS — Real Data  [DB-HLSTM — GPU P100]")
...
# ============================================================
# STEP 2 — 20-CLASS LABELING VIA KMEANS ON CV
# ============================================================
...
for cls in range(N_CLASSES):
    cnt     = (df['Class20']==cls).sum()
    cv_mean = df[df['Class20']==cls]['I_val'].mean()
    print(f"    Class {cls:2d}: {cnt:5,}  MeanCV={cv_mean:.3f}")
...
print(f"  y_20 unique: {np.unique(y_20[:idx])}")  # must be [0..19]
...
# ============================================================
# STEP 7 — 20-CLASS TRAINING
# ============================================================
print("\n" + "=" * 60)
print("  20-CLASS CLASSIFICATION")
print("=" * 60)

y_cat = to_categorical(y_20, num_classes=N_CLASSES)
del y_20; gc.collect()

tr_m, vl_m, te_m, yte_m = make_datasets(X_norm, X_sc, y_cat, BATCH_SIZE)
del X_norm, X_sc, y_cat; gc.collect()

model_m = build_model(N_CLASSES)
model_m.summary()

cb_m = [
    ModelCheckpoint(MULTI_MODEL, monitor='val_accuracy',
    ...
    CSVLogger('/kaggle/working/multiclass_real_log.csv')
]

model_m.fit(
    tr_m, validation_data=vl_m,
    epochs=EPOCHS, callbacks=cb_m,
    verbose=1
)

res_m = model_m.evaluate(te_m, verbose=0)
print()
print("=" * 60)
print("  20-CLASS RESULTS")
print("=" * 60)
print(f"  Accuracy  : {res_m[1]*100:.2f}%")
print(f"  Loss      : {res_m[0]:.4f}")
if   res_m[1]>=0.85: print("  STATUS : Excellent ✅")
elif res_m[1]>=0.70: print("  STATUS : Good ✅")
elif res_m[1]>=0.55: print("  STATUS : Acceptable ✅")
else:                print("  STATUS : Needs improvement ⚠️")
print("=" * 60)

del model_m, tr_m, vl_m, te_m
gc.collect()
tf.keras.backend.clear_session()

print("\n  20-CLASS DONE ✅")
print(f"  Model saved: {MULTI_MODEL}")
print(f"  Log saved  : /kaggle/working/multiclass_real_log.csv")
